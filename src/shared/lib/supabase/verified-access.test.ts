...
beforeEach(() => {
  ...
  mocks.cookieGet.mockImplementation((name: string) => ({
    value: name === "demo_role" ? "admin" : "admin@blueteam.com",
  }));
  mocks.profile = {
    data: {
      role: "admin",
  ...
  mocks.from.mockImplementation((table: string) => {
    ...
    query.maybeSingle = vi.fn(() => {
      profileLookup = table === "profiles";
      return query;
    });
    query.then = (resolve: (result: unknown) => unknown) =>
      Promise.resolve(profileLookup ? mocks.profile : mocks.result).then(
        resolve,
      );
    return query;
  });
...
describe("verified access", () => {
  ...
  it.each([null, { role: "owner" }])(
    "rejects missing or invalid persisted profile %j",
    async (profile) => {
  ...
  it("retains the persisted student role despite forged admin hints", async () => {
    ...
    expect(await getResilientUser()).toMatchObject({
      isDemo: false,
...
describe("academic data provenance", () => {
  ...
  it.each(readers)(
    "returns empty real %s without adding simulated data",
    async (_, read) => {
  ...
  it.each(readers)(
    "propagates %s database errors rather than inventing a result",
    async (_, read) => {
