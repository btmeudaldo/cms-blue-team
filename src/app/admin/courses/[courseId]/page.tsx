      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
          <div>
            ...
            <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">Editar Curso &amp; Lecciones</h1>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-6">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-4">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Ajustes del Curso</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Modifica los metadatos generales.</p>
            </div>

            <form action={updateThisCourse} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Título</label>
                <input
                  name="title"
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {lessons.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 p-12 text-center shadow-xs">
                ...
                <h3 className="text-base font-bold text-slate-900 dark:text-white">Este curso no tiene lecciones creadas</h3>
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
                  Agrega la primera lección para incluir contenido enriquecido con imágenes y texto anticheat.
                </p>
                <Link
                  href={`/admin/courses/${course.id}/lessons/new`}
              ...
              <div className="space-y-3">
                {lessons.map((lesson: any) => {
                  const deleteThisLesson = deleteLessonAction.bind(null, lesson.id, course.id);
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                            {lesson.word_count || 0} palabras &middot; Tiempo mín. exigido:{" "}
                            <span className="font-bold text-[#1a80ff]">{lesson.min_seconds}s</span>
                          </p>
                        </div>
                      </div>
