...
import { createNewUserAction, updateUserRoleAction } from "@/app/actions/enrollment.actions";
import { getResilientCourses, getResilientProfiles, getResilientUser } from "@/shared/lib/supabase/resilient";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">
              ...
              <span>&rsaquo;</span>
              <span className="text-slate-900 dark:text-white">Matrículas &amp; Usuarios</span>
            </div>
            <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">Gestión de Usuarios y Asignaciones</h1>
          </div>
          ...
          {isFullAdmin ? (
            <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-6">
              <div className="border-b border-slate-100 dark:border-slate-800 pb-4">
                ...
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Registrar Nuevo Usuario</h2>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Crea pilotos alumnos o instructores de vuelo.</p>
              </div>
            ...
            <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-3">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Como <strong>Instructor de Vuelo</strong> puedes matricular y asignar cursos a los alumnos. La creación de usuarios y cambio de roles de sistema es de gestión exclusiva del <strong>Director de la Escuela</strong>.
              </p>
            </div>
          )}
                ...
                <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
                  <thead className="bg-slate-50 dark:bg-slate-800/60 text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 border-b border-slate-200 dark:border-slate-800">
                    <tr>
                      ...
                      <th className="px-6 py-4">Matrículas Asignadas (Desplegable Escalable)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {profiles.map((u: any) => {
                      ...
                      const enrolledCourseIds = (courses || [])
                        .filter((c: any) => mockStore.isEnrolled(u.id, c.id))
                        .map((c: any) => c.id);
                            ...
                            {isFullAdmin ? (
                              <form
                                action={async (formData: FormData) => {
                                  "use server";
                                  const newRole = String(formData.get("role") ?? "student") as any;
                              ...
                              >
                                {u.role === "admin" ? "Director / Admin" : u.role === "instructor" ? "Instructor de Vuelo" : "Piloto Alumno"}
                              </span>
                            )}
                          </td>
