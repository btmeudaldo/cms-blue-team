...
import { getResilientCourses, getResilientUser, getResilientUserProgress } from "@/shared/lib/supabase/resilient";
...
export default async function CoursesPage() {
  ...
  const completedLessonIds = new Set(
    userProgress?.filter((p: any) => p.is_completed).map((p: any) => p.lesson_id) || []
  );
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0b1120] text-slate-900 dark:text-slate-100 flex flex-col transition-colors">
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
          <div>
            <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white">Mis Cursos</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              {coursesData.length} {coursesData.length === 1 ? "curso disponible" : "cursos disponibles"}
            </p>
          </div>
        ...
        {coursesData.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 p-12 text-center shadow-xs">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">No tienes cursos asignados aún</h3>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 max-w-md mx-auto">
              Puedes hacer clic en el botón de abajo &quot;Cargar Curso Demo Ahora&quot; para generar e inscribirte instantáneamente en módulos con lecciones teóricas y temporizadores anticheat.
            </p>
            <div className="mt-6">
              <SeedDemoButton label="Cargar Curso Demo Ahora" variant="primary" />
            </div>
          </div>
        ) : (
