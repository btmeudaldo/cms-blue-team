...
const COURSE_ID = "17200000-0000-0000-0000-000000000172";
const COURSE_SLUG = "cessna-172-continental-diesel";
const LESSON_1_ID = "17200000-0000-0000-0000-000000000001";
const LESSON_1_SLUG = "celula-mandos-limitaciones-flota";

const lesson1ContentHtml = `
...
async function main() {
  console.log("Creating/Updating course and seeding Lesson 1...");
  ...
  const { error: courseError } = await supabase.from("courses").upsert(
    {
      ...
      slug: COURSE_SLUG,
      description: "Curso oficial de familiarización de tipo, diferencias técnicas y procedimientos de emergencia para la flota Cessna 172 con motores Continental CD-135 y CD-155 Turbo Diésel (EC-OXT, EC-OXV, EC-NNA, EC-NNX) de Blue Team Flight School, conforme a EASA Part-FCL.710.",
  ...
  console.log("Course successfully upserted:", COURSE_ID);

  // 2. Ensure Lesson 1 Exists with exact content
  const wordCount = lesson1ContentHtml.split(/\s+/).filter(Boolean).length;
  ...
  const { error: lessonError } = await supabase.from("lessons").upsert(
    {
      ...
      content_html: lesson1ContentHtml,
    },
    { onConflict: "id" }
  );
  ...
  console.log("Lesson 1 successfully upserted:", LESSON_1_ID);

  // 3. Ensure admin and test student are enrolled
  const profiles = ["0cff202a-c552-4c1f-ae31-a2bc1635cac1", "b6448edc-99de-4c67-bd9b-3f137f95def2", "64b65071-b214-4132-8cbd-00cba2f05621"];
